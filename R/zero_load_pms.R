.mle1d_pms <- function(V,Y,left,value_only=TRUE){
  ...
  ll <- -log_dhurdle1d_pms(V,Y,phat,muhat,sigmasqhat)
  if (value_only)
    return (ll)
  else{
...
.zi_fit_lm_sub <- function(y, X, binomial, intercept, penalize,
  ...
  if (!penalize) {
    ...
    return (list("coefficients"=model$coefficients,
                 "logLik"=stats::logLik(model), "sigmasq"=sigmasq,
...
#'
#' @export
zi_fit_lm <- function(y, X, binomial, intercept=TRUE, tol=1e-8,
  ...
  if (penalize) { # If fitting penalized regression OR tbe unpenalized regression was bad
    best_res <- .zi_fit_lm_sub(y, X, binomial, intercept, penalize=TRUE,
...
.res_from_model_fits <- function(p_model, mu_model, n, left_name, value_only){
  ...
  best_res <- list()
  best_res$value <- as.numeric(-(p_model$logLik+mu_model$logLik)/n) # Negative mean log likelihood
  if (value_only)
